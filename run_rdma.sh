#### testing AMem NCCL Plugin in GPU + RDMA case (single node)
#### the node should have RDMA hardware && RDMA device enabled && lib service ready 
set -x
...
### disable P2P to test RDMA within a single node, assuming RDMA NIC is available
...
for test in ${TESTS}; do
  ...
  mpirun --allow-run-as-root -bind-to none \
    ...
    -x NCCL_DEBUG=INFO \
    ./build/${test} -b 1M -e 8M -f 2 -g 1 -n 10
