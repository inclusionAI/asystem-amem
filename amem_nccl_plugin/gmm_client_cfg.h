...
struct gmm_client_cfg {
  ...
  //int groupID; //a logical ID that shared across workers/client within a job
 ...
 public:
  gmm_client_cfg() {
    ...
    int groupID = getenv("AMEM_GROUPID") ? atoi(getenv("AMEM_GROUPID")) : 0;
    printf("AMEM groupID:%d f:%s %s %d pid:%d dev_cnt:%d init gmm cfg\n", groupID, __FILE__, __FUNCTION__, __LINE__, getpid(), dev_cnt); fflush(stdout);
    //dev_cnt = 4; // TODO: remember to 1) remove this if used all GPUs in the node;  2) or update to the actual cnt if use partial GPUs
  }
